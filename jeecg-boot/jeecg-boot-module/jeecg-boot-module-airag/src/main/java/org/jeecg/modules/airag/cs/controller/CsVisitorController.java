...
public class CsVisitorController extends JeecgController<CsVisitor, ICsVisitorService> {
    ...
    public Result<CsVisitor> getByUser(
        ...
        CsVisitor visitor = visitorService.getByAppAndUser(appId, userId);
        if (visitor == null) {
            return Result.error("访客不存在");
        }
        return Result.OK(visitor);
    ...
    @PostMapping("/update")
    public Result<String> update(@RequestBody CsVisitor visitor) {
    ...
    public Result<String> toggleStar(@RequestParam String id) {
        boolean success = visitorService.toggleStar(id);
    ...
    public Result<String> quickRemark(
            @RequestParam String id,
